...
yt-dlp --extract-audio --embed-thumbnail -o "music/%(artist)s/%(album)s/%(playlist_index)s - %(title)s.%(ext)s" --audio-format mp3 --add-metadata  $1 --ppa "EmbedThumbnail+ffmpeg_o:-c:v mjpeg -vf crop=\"'if(gt(ih,iw),iw,ih)':'if(gt(iw,ih),ih,iw)'\""
