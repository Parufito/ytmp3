...
yt-dlp --extract-audio --restrict-filenames --embed-thumbnail -o "music/single/%(title)s.%(ext)s" --audio-format mp3 --add-metadata  $1 --ppa "EmbedThumbnail+ffmpeg_o:-c:v mjpeg -vf crop=\"'if(gt(ih,iw),iw,ih)':'if(gt(iw,ih),ih,iw)'\""
